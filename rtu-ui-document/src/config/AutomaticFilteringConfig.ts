...
export function applyAutomaticFiltering(config: UIConfig.DocumentScanningFlow) {
    ...
    config.outputSettings.defaultFilter = new ScanbotSDK.UI.Config.ParametricFilters.ScanbotBinarizationFilter();

    //or you can set custom filter with custom values
    config.outputSettings.defaultFilter = new ScanbotSDK.UI.Config.WhiteBlackPointFilter({
        blackPoint: 0.1,
        whitePoint: 0.9
    });
}