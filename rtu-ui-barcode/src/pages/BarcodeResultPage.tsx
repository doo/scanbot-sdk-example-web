...
export default function BarcodeResultPage() {
    ...
    return (
        <Box sx={{ width: '100%', height: "100%", bgcolor: 'background.paper' }}>
            ...
            {SBStorage.instance.getBarcodes()?.map((item, index) => {
                return (
                    <Box key={index} sx={{ padding: 2, borderBottom: "1px solid #e0e0e0" }}>
                        <Box sx={{ display: "flex", flexDirection: "row" }}>
                            <Box sx={{ fontWeight: 500, fontSize: 14, color: "rgb(200, 25, 60)" }}>
                                {item.barcode.text}
                            </Box>
                            <Box sx={{ paddingLeft: 1, fontSize: 14, color: "rgb(100, 100, 100)" }}>
                                {"(" + item.barcode.format + ")"}
                            </Box>
                            <Box sx={{ paddingLeft: 1, fontSize: 14, color: "rgb(0, 0, 0)" }}>
                                {"x" + item.count}
                            </Box>
                        </Box>
                    </Box>
